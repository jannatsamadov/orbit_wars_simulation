...
# ==============================================================================
# 6. ADVANCED STRATEGY (DeepSeek)
# ==============================================================================
def generate_strategy_moves(obs, player, my_sources, all_targets, timeline, k_state):
    """
    High-level strategic layer that selects optimal fleet launches.
    
    Uses concepts:
    - Discounted future value of planets (production over time)
    - Risk adjustment via distance to enemies (frontline/backline)
    - Economic disruption: prioritise attacking the strongest opponent
    - Logistics: backline planets without good attacks reinforce frontlines
    
    The underlying `find_valid_plan` guarantees kinematic correctness.
    """
    moves = []
    committed = {src.id: 0 for src in my_sources}
    my_sources_by_id = {src.id: src for src in my_sources}

    # ---------- 1. Global board analysis ----------
    # Determine opponents
    opponents = set()
    for t in all_targets:
        if t.owner not in (-1, player):
            opponents.add(t.owner)
    # Total production and fleet strength per opponent
    opp_production = {op: 0 for op in opponents}
    opp_total_ships = {op: 0 for op in opponents}
    for t in all_targets:
        if t.owner in opponents:
            opp_production[t.owner] += t.production
            opp_total_ships[t.owner] += t.ships
    # Also count ships on planets we own (for own strength)
    my_total_ships = sum(src.ships for src in my_sources)
    my_total_prod = sum(src.production for src in my_sources)

    # Identify strongest opponent (by total production + ships)
    strongest_opp = max(opponents, key=lambda op: opp_production[op] + 0.5*opp_total_ships[op], default=None)

    # Planet valuations – for defence and capture scoring
    VALUE_PER_PRODUCTION = 60.0   # rough estimate of game length
    DISCOUNT_RATE = 0.05          # per turn
    FRONTLINE_RISK_ALPHA = 0.15   # higher alpha = more risk near enemies

    def planet_base_value(p):
        """Intrinsic long-term value of owning a planet (ignoring current owner)."""
        return p.production * VALUE_PER_PRODUCTION

    # Precompute risk factors for all planets: how exposed to enemies?
    # For an enemy planet we might capture: high exposure = harder to hold
    # For our planets, exposure = how likely to be attacked.
    risk_factor = {}
    for p in all_targets:
        # Minimum distance to any enemy planet (excluding the planet itself if enemy)
        min_dist = 100.0
        for ep in all_targets:
            if ep.owner not in (-1, player) and ep.id != p.id:
                d = math.hypot(p.x - ep.x, p.y - ep.y)
                if d < min_dist:
                    min_dist = d
        # Closer to enemy -> lower safety factor
        risk_factor[p.id] = math.exp(-FRONTLINE_RISK_ALPHA * min_dist)

    # For our own planets, compute a defend value (cost of losing them)
    own_planet_value = {}
    for src in my_sources:
        # Value of planet = base value * current risk (lower if safe)
        own_planet_value[src.id] = planet_base_value(src) * risk_factor.get(src.id, 1.0)

    # ---------- 2. Generate all candidate plans ----------
    Candidate = namedtuple('Candidate', ['score', 'src_id', 'tgt_id', 'plan', 'is_attack', 'tgt_owner'])
    candidates = []

    for src in my_sources:
        avail = src.ships
        # No ships to send – skip
        if avail <= 0:
            continue

        for tgt in all_targets:
            # Skip self (source == target) – reinforcement will be handled separately
            if tgt.id == src.id:
                continue
            ...
            for plan in plans:
                req_ships = plan['ships']
                if req_ships > avail:
                    continue  # defensive check, though engine should handle
                T_arrival = plan['T']
                discount = 1.0 / ((1.0 + DISCOUNT_RATE) ** T_arrival)

                if tgt.owner == player:
                    # ----- Defensive reinforcement -----
                    # Only valuable if threat exists (plan suggests defence needed)
                    # Score = value of saving the planet minus cost of ships
                    planet_value = own_planet_value.get(tgt.id, 0)
                    urgency = 1.0  # find_valid_plan returned a plan, so threat is real
                    gain = planet_value * urgency * discount
                    cost = req_ships
                    score = gain - cost
                    candidates.append(Candidate(score, src.id, tgt.id, plan, False, tgt.owner))
                else:
                    # ----- Offensive capture -----
                    base_value = planet_base_value(tgt)
                    # Modifier for attacking the strongest opponent
                    disruption = 0.0
                    if tgt.owner == strongest_opp and strongest_opp is not None:
                        disruption = opp_production[strongest_opp] * 0.2  # bonus
                    # Risk of holding: captured planet becomes our front line
                    safety = risk_factor.get(tgt.id, 1.0)
                    # Value gained = (production over future) * safety + disruption
                    gain = (base_value * discount * safety) + disruption
                    cost = req_ships
                    score = gain - cost
                    candidates.append(Candidate(score, src.id, tgt.id, plan, True, tgt.owner))

    # ---------- 3. Select moves with constrained greedy ----------
    # Sort by score descending
    candidates.sort(key=lambda c: c.score, reverse=True)

    attacked_targets = set()   # enemy/neutral targets we already assigned an attack
    used_src_ships = {src.id: 0 for src in my_sources}

    for c in candidates:
        # Check source has enough uncommitted ships
        if used_src_ships[c.src_id] + c.plan['ships'] > my_sources_by_id[c.src_id].ships:
            continue

        # For attacks: only one source per enemy target (avoid over-saturating)
        if c.is_attack:
            if c.tgt_id in attacked_targets:
                continue
            attacked_targets.add(c.tgt_id)
        # For defence: we allow multiple sources to reinforce the same friendly planet

        # Commit
        used_src_ships[c.src_id] += c.plan['ships']
        moves.append([int(c.src_id), float(c.plan['angle']), int(c.plan['ships'])])
...
def agent(obs) -> list:
    ...
    # 3. Use DeepSeek generated strategy
