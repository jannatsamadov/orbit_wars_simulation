...
def agent(obs) -> list:
    ...
    for src in sorted(my_sources, key=lambda p: p.ships, reverse=True): # Ən çox gəmisi olandan başla
        ...
        for tgt in all_targets:
            ...
            for p in plans:
                ...
                value = tgt.production * (timeline.SIM_HORIZON - T_actual)
                # Kometlərin dəyərini xüsusi hesablayırıq (0 production, amma faydalı mövqe ola bilər)
                if tgt.production == 0:
                    value = 50 # Base dəyər komet üçün
                    
                if tgt.owner != player:
                    score = value / max(1, S_final)
                else:
                    S_combat = timeline.get_cost_to_capture(tgt.id, T_actual, player)
                    if S_combat > 0: # Düşmən alır, qorumalıyıq!
                        score = (value / max(1, S_final)) * 1.5
                    else:
                        continue # Artıq güvəndədir
        ...
        if best_target and best_plan is not None:
            moves.append([int(src.id), float(best_plan["angle"]), int(best_plan["ships"])])
            committed_ships[src.id] += best_plan["ships"]
